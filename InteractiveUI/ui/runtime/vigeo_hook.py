...
def install() -> None:
    ...
    def ensure_estimator(device: torch.device):
        global _ESTIMATOR
        if os.environ.get('EVOKE_AUX_GPU') is not None:
            device=torch.device('cuda',int(os.environ['EVOKE_AUX_GPU']))
    ...
    def move_to(self, *args, **kwargs):
        ...
        estimator._lazy()
        latency_queue = os.environ.get('EVOKE_ARGV_SERVER_DIR')
        if latency_queue:
            from ui.vigeo_latency import install as install_latency
            install_latency(estimator, Path(latency_queue))
