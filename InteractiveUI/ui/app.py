...
try:
    from .trajectory import OUTPUT_FPS, POSE_FPS, SOURCE_HEIGHT, SOURCE_WIDTH, append_pose_npz, build_pose_npz, resample_pose_npz, validate_camera_samples
...
CONTROL_ONLY = os.environ.get("EVOKE_UI_CONTROL_ONLY", "0") == "1"
...
class Job:
    ...
    live_resolution: tuple[int, int] = (384, 640)
    ...
    def snapshot(self) -> dict:
        ...
        browser_final = _ensure_browser_video(final_source, self.root) if final_ready else None
        ...
        return {
            ...
            "finalVideo": f"api/jobs/{self.id}/files/{browser_final.relative_to(self.root)}" if browser_final else None,
...
async def _restart_job_worker(job: Job) -> None:
...
def _validate_spec(raw: str) -> dict:
    ...
    if spec.get("cameraSamples") is not None:
        try:
            validate_camera_samples(spec["cameraSamples"], chunks)
...
async def _ensure_final_revision(job: Job) -> Path:
    ...
    async with lock:
        ...
        marker_path = job.root / "final_from_segments.json"
        signature = [[str(path.relative_to(job.root)), path.stat().st_size, path.stat().st_mtime_ns]
        ...
        if not final_path.is_file() or final_path.stat().st_size <= 0 \
                or marker.get("segments") != signature \
                or marker.get("finalMtimeNs") != final_path.stat().st_mtime_ns:
            await asyncio.to_thread(_finalize_revision, job)
            if final_path.is_file():
                _write_json(marker_path, {"segments": signature, "finalMtimeNs": final_path.stat().st_mtime_ns})
        return final_path
        ...
        env.update(
            {
                ...
                "SAVE_SEGMENTS": "1",
                "IN_PROCESS_BATCH": "1",
                "EVOKE_ARGV_SERVER_DIR": str(job.worker_root or WORKER_ROOT),
                "EVOKE_SERVER_REQUEST_ID": job.id,
            }
        )
        _apply_model_environment(env)
        if job.live_root is not None:
            env.update(EVOKE_LIVE_CONTROL_PATH=str(job.live_root), SAVE_SEGMENTS="0", DUMP_GEO="0",
...
async def extend_job(job_id: str, payload: dict = Body(...)) -> dict:
    ...
    if payload.get("cameraSamples") is not None:
        try:
            validate_camera_samples(payload["cameraSamples"], additional)
        except ValueError as error:
            raise HTTPException(400, str(error)) from error
        ...
        _write_json(revision_root / "spec.json", {
            **parent_spec,
            ...
            "continuation": {
                ...
                "cameraSamples": payload.get("cameraSamples"),
...
async def cancel_job(job_id: str) -> dict:
    ...
    if job.process and job.process.returncode is None:
        ...
        asyncio.create_task(_restart_job_worker(job))
...
async def download_job(job_id: str) -> FileResponse:
    ...
    target = await asyncio.to_thread(_ensure_browser_video, target, job.root)
...
import sys as _sys
try:
    from .live_api import install as _install_live
except ImportError:
    from live_api import install as _install_live
_install_live(app, _sys.modules[__name__])
