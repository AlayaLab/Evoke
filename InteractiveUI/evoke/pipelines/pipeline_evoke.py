...
class EvokePipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def _decode_chunk_persistent_cache(
        ...
        import os as _os
        import time as _decode_time
        _decode_started = _decode_time.perf_counter()
        ...
        if warm_n > 0:
            warm = warm_latents[:, :, -warm_n:].to(z_chunk.dtype).to(z_chunk.device)
        ...
        for i in range(x.shape[2]):
            ...
            live = getattr(self, "_evoke_live", None)
            if live is not None and getattr(live, 'vae_precision', 'fp32') == 'fp16_mixed':
                from ui.vae_precision_runtime import prepare_decoder as prepare_mixed_decoder
                prepare_mixed_decoder(self.vae)
            if live is not None and getattr(live, 'precision_probe', None):
                from ui.vae_precision_probe import prepare_decoder
                prepare_decoder(self.vae, live)
            if live is not None and getattr(live, 'precision_trial', None):
                from ui.vae_precision_probe import prepare_trial
                prepare_trial(self.vae, live)
            if live is not None:
                _decode_begin=torch.cuda.Event(enable_timing=True)
                _decode_end=torch.cuda.Event(enable_timing=True)
                _decode_begin.record()
                _slice_host_started=_decode_time.perf_counter()
            _vae_options=getattr(live,'vae_decode_options',{}) if live is not None else {}
            if live is not None and getattr(live, 'vae_precision', 'fp32') == 'fp16_mixed':
                from ui.vae_precision_runtime import decode_slice as decode_mixed_slice
                slice_out,_vae_stats=decode_mixed_slice(self.vae,x[:,:,i:i+1],first_frame_flag,_vae_options)
                live.vae_decode_stats.append(_vae_stats)
            elif live is not None and getattr(live, 'precision_trial', {}).get('decoder'):
                from ui.vae_precision_probe import trial_decode
                slice_out,_vae_stats=trial_decode(self.vae,x[:,:,i:i+1],first_frame_flag,live)
                live.vae_decode_stats.append(_vae_stats)
            elif _vae_options:
                from ui.vae_decode_runtime import decode_slice as _optimized_decode_slice,probe_group
                _group_probe=None
                if _vae_options.get('groupProbe') and live.chunk==2 and i==1 and i+2<=x.shape[2]:
                    _group_probe=probe_group(self.vae,x[:,:,i:i+2])
                slice_out,_vae_stats=_optimized_decode_slice(self.vae,x[:,:,i:i+1],
                    first_chunk=first_frame_flag,options=_vae_options,
                    verify=bool(_vae_options.get('verify')) and live.chunk in (0,1,2,8,32,55,95),
                    profile=bool(_vae_options.get('profile')) and live.chunk in (2,20) and i in (0,4),
                    benchmark=bool(_vae_options.get('benchmark')) and live.chunk in (2,20,60) and i==4)
                if _group_probe is not None:_vae_stats['groupProbe']=_group_probe
                live.vae_decode_stats.append(_vae_stats)
            elif live is not None and getattr(live,'decode_graph',False) and not is_first_chunk:
                from ui.vae_decode_graph import decode_slice
                slice_out,_graph_stats=decode_slice(self.vae,x[:, :, i : i + 1, :, :],
                    first_chunk=first_frame_flag,enabled=True,
                    verify=live.verify_decode and live.chunk in (0,1,2,3,8,32,55,95))
                live.decode_graph_stats.append(_graph_stats)
            else:
                ...
                if live is not None and getattr(live,'decode_graph',False):


                    live.decode_graph_stats.append({'graphRequested':True,'graphUsed':False,
                        'graphInitialized':False,'verified':False,'eagerReason':'first-window'})
            if live is not None:
                _decode_end.record()
                live.decode_events.append((_decode_begin,_decode_end))
                if getattr(live,'latency_profile',False):
                    live.decode_host_seconds.append(_decode_time.perf_counter()-_slice_host_started)
            if live is not None and getattr(live, 'precision_probe', None):
                from ui.vae_precision_probe import shadow_decode
                shadow_decode(self.vae, x[:,:,i:i+1], first_frame_flag, slice_out, live, i)
            if live is not None and getattr(live, 'precision_trial', None):
                from ui.vae_precision_probe import save_trial_pixels
                save_trial_pixels(slice_out,live,i)
            out_list.append(slice_out)
            live = getattr(self, "_evoke_live", None)
            if live is not None and i >= warm_n:
                live_slice = slice_out
                if getattr(self.vae.config, "patch_size", None) is not None:
                    from diffusers.models.autoencoders.autoencoder_kl_wan import unpatchify
                    live_slice = unpatchify(live_slice, patch_size=self.vae.config.patch_size)
                live.decoded(live_slice)
        ...
        result = torch.clamp(out, min=-1.0, max=1.0)
        live = getattr(self, "_evoke_live", None)
        if live is not None:
            live.decode_seconds += _decode_time.perf_counter() - _decode_started
        return result
    ...
    def _geo_da3_setup(self, geo_state, cfg, lingbot_Ks, height, width, pix_stride, device):
        ...
        import numpy as _np
        import os as _geo_os
        if _geo_os.environ.get('EVOKE_AUX_GPU') is not None:
            device=torch.device('cuda',int(_geo_os.environ['EVOKE_AUX_GPU']))
        kn = lingbot_Ks.detach().cpu().numpy().reshape(-1)[:4]
        ...
        _est_key = (_backend.lower(), str(device), pr, str(cfg.get("da3_weights") or ""),
        ...
        geo_state["da3_bank"] = DA3FrameBank(
            ...
            consist_probation_frac=float(cfg.get("consist_probation_frac", 0.0)),
            ...
            self_reanchor_anchor_min_conf=float(cfg.get("self_reanchor_anchor_min_conf", 0.0)),
            self_reanchor_pin_prime=bool(cfg.get("self_reanchor_pin_prime", False)),
            ...
            carve_min_views=int(cfg.get("carve_min_views", 1)),

            carve_strike_windows=int(cfg.get("carve_strike_windows", 1)),
        )
        if bool(cfg.get("color_anchor", False)):
    ...
    def _geo_da3_ingest(self, geo_state, frames_local, global_pix_start, c2w_full, device):
        ...
        frames01 = (frames_local[:, loc].permute(1, 2, 3, 0) * 0.5 + 0.5).clamp(0, 1).float().cpu().numpy()
    ...
    def _geo_render_chunk_da3(self, geo_state, chunk_idx, camera_poses_pix_window, height, width,
        ...
        if _keys:
            if geo_state.get('da3_fixed_lag'):
                from ui.geometry_overlap import history_cutoff
                _cutoff = history_cutoff(geo_state['da3_target_start'],stride,lag)
            else:
        ...
        if _chunk0_ref:
            ...
            _ref = source_frame_pix[0] if source_frame_pix.ndim == 4 else source_frame_pix
            _ref_pose = tgt[0]
            ...
            if render_mode == 'backward_zbuf':
                _render_bw = geo_state.get('multisrc_renderer', _render_bw)
        ...
        if _dump_dir is not None:
            try:
                ...
                _vt = visibility_mask[0, 0].clamp(0, 1)
        ...
        self._geo_last_render_stats = {"pool": len(pool), "render_mode": render_mode,
                                       ...
                                       "historyCutoff": _cutoff if _keys else None,
                                       "poolMin": min(pool) if pool else None, "poolMax": max(pool) if pool else None}
        if not getattr(self, "_geo_quiet_chunk_logs", False):
    ...
    def _geo_render_chunk(
        ...
        width: int,
        device: torch.device,
        anchor_c2w: Optional[torch.Tensor] = None,
        top_k: int = 8,
        nearby_k: int = 0,
    ...
    def _geo_encode_warp_to_latents(
        ...
        encoded_moments=None,
    ) -> torch.Tensor:

        _warm_enc = bool((getattr(self, "_geo_vsnoise_cfg", None) or {}).get("geo_warp_warm_encode", False))
        if encoded_moments is not None:
            from diffusers.models.autoencoders.vae import DiagonalGaussianDistribution

            DiagonalGaussianDistribution(encoded_moments[0]).sample(generator=generator)
            warp_latents=DiagonalGaussianDistribution(encoded_moments[1]).sample(generator=generator)
            warp_latents=((warp_latents-latents_mean)*latents_std).to(device=device,dtype=torch.float32)
        elif _warm_enc:
    ...
    def _geo_even_threshold_map(self, Tn: int, H: int, W: int, g: int, device: torch.device) -> torch.Tensor:
        ...
        tile = base.repeat((gh + n - 1) // n, (gw + n - 1) // n)[:gh, :gw]
        out = torch.empty(Tn, gh, gw, device=device)
        for t in range(Tn):
            out[t] = torch.roll(tile, shifts=((t * 3) % n, (t * 5) % n), dims=(0, 1))
        return out.repeat_interleave(g, -2).repeat_interleave(g, -1)[:, :H, :W]
    ...
    def _geo_build_short_visibility_mask(
        ...
        if 0.0 < _cap < 1.0 and sampled.numel() > 0 and _Tn > 0:
            ...
            _vis = sampled.mean(dim=(0, 1, 3, 4)).clamp_min(1e-6)
            _keep_frac = (_cap / _vis).clamp(max=1.0)
            _thr = self._geo_even_threshold_map(_Tn, H_lat, W_lat, _g, sampled.device)
            _keep = (_thr < _keep_frac.view(_Tn, 1, 1)).to(sampled.dtype)
        ...
        if not (0.0 < _cap < 1.0) and _pdr > 0.0 and sampled.numel() > 0:
            ...
            _pd = _pd.repeat_interleave(_g, -2).repeat_interleave(_g, -1)[:, :H_lat, :W_lat]
    ...
    def stage1_sample(
        ...
        nearby_sink_indices: torch.Tensor = None,
    ...
    def stage2_sample(
        ...
        cam_lingbot_Ks_chunk: Optional[torch.Tensor] = None,
        cam_lingbot_c2ws_chunk: Optional[torch.Tensor] = None,
        ...
        for i_s in range(stage2_num_stages):
            ...
            if (
                ...
                _base_h = cam_base_height_pix or _H_pix_stage
    ...
    def __call__(
        ...
        if chunk_prompts:
            _uniq = {prompt: all_prompt_embeds} if isinstance(prompt, str) else {}
        ...
        live_control = getattr(self, "_evoke_live", None)
        _live_prompt_conditioning = None
        if live_control is not None:
            from ui.live_prompt import LivePromptConditioning
            if self.text_encoder.training:
                raise ValueError("Live prompt updates require the text encoder in eval mode")
            def _encode_live_prompt(text):


                token_ids = self.tokenizer(prompt_clean(text), add_special_tokens=True, truncation=False,
                                           return_attention_mask=False)["input_ids"]
                if len(token_ids) > max_sequence_length:
                    raise ValueError(f"描述过长（{len(token_ids)} tokens，上限 {max_sequence_length}），请缩短事件或使用替换完整描述。")
                embeds, _, _, _ = self.encode_prompt(
                    prompt=text, negative_prompt=None, do_classifier_free_guidance=False,
                    num_videos_per_prompt=num_videos_per_prompt,
                    max_sequence_length=max_sequence_length, device=device)
                embeds = embeds.to(transformer_dtype)
                if not bool(torch.isfinite(embeds).all()):
                    raise ValueError("新描述编码结果无效，已保留之前的描述。")
                return embeds
            _live_prompt_conditioning = LivePromptConditioning(
                live_control, prompt, all_prompt_embeds, _encode_live_prompt)


        if live_control is None and bool(getattr(self, "_evoke_offload_text_encoder", False)):
        ...
        if use_geometric_state:
            ...
            _geo_pix_stride_seed = latent_window_size * self.vae_scale_factor_temporal
            ...
            if _is_v2v_geo:
                ...
                _geo_anchor_image = video[0, :, -1].contiguous()
            ...
            if _is_v2v_geo:
                ...
                _seed_max_extra = _geo_init_max - 1
                ...
                for _pi in _seed_pixel_idxs:
                    if _pi == _geo_source_pixel_idx_seed:
                        continue
                    _seed_pix = video[0, :, _pi].detach().float().cpu()
            ...
            if str(_da3_cfg.get("recon_backend", "da3")) == "da3" and lingbot_Ks is not None:
                _da3_stride = latent_window_size * self.vae_scale_factor_temporal
                ...
                if _is_v2v_geo and video is not None:
        ...
        if live_control is not None:
            import itertools
            _chunk_iter = itertools.count()
        _prog_bar = None
        if _prog_on and live_control is None:
            import sys as _psys
        ...
        _live_geometry = None
        if live_control is not None and use_geometric_state:
            from ui.geometry_overlap import GeometryOverlap
            if _event_set or _oracle_reanchor_every():
                raise ValueError('Live geometry overlap does not support event or oracle reanchor modes')
            if live_control.geometry_mode in {'dedicated','prefetch'}:
                from ui.warp_prefetch import WarpPrefetch
                _live_geometry = WarpPrefetch(live_control,self,geo_state,device)
            else:
                _live_geometry = GeometryOverlap(live_control, self, geo_state, device)
            live_control.geometry = _live_geometry

        for k in _chunk_iter:
            if live_control is not None:
                if _live_geometry is not None:_live_geometry.before_warp(k)
                live_poses = live_control.next_poses(k, latent_window_size * self.vae_scale_factor_temporal)
                if live_poses is None:
                    break
                if _live_geometry is not None:


                    live_control.chunk_started -= _live_geometry.wait_seconds
                live_poses = torch.from_numpy(live_poses).unsqueeze(0).to(device=device, dtype=torch.float32)
                geo_c2ws_dev = live_poses
                if cam_c2ws_dev is not None:
                    cam_c2ws_dev = live_poses
            self._evoke_progress_chunk_index = k
            self._evoke_progress_chunks = num_latent_sections
            _t_chunk0 = _ptime.time()
            _t_warp_s = None
            _t_diff_s = None
            self._geo_last_render_stats = None
            ...
            if _live_prompt_conditioning is not None:
                _live_embeddings = _live_prompt_conditioning.for_chunk()
                if live_control.prompt_revision > 0:
                    prompt_embeds = _live_embeddings
            ...
            if use_geometric_state:
                ...
                _geo_pix_stride = latent_window_size * self.vae_scale_factor_temporal
                _geo_window_pix = window_num_frames
                _geo_pix_start = geo_ref_pose_offset_pix + _geo_pose_k * _geo_pix_stride
                if _live_geometry is not None:geo_state['da3_target_start'] = int(_geo_pix_start)
                _geo_pix_count = _geo_window_pix
                ...
                if is_first_section:
                    ...
                    if video is not None and isinstance(video, torch.Tensor) and video.ndim == 5 and int(cam_video_lat_offset) > 0:
                        _geo_source = video[:, :, -1].to(device=device, dtype=torch.float32)
                ...
                if is_event_k:
                    ...
                    if _live_geometry is not None and hasattr(_live_geometry,'render_and_encode'):
                        _geo_visibility_mask,_geo_warp_latents=_live_geometry.render_and_encode(
                            dict(geo_state=geo_state,chunk_idx=k,source_frame_pix=_geo_source,
                                 camera_poses_pix_window=_geo_pose_window,height=height,width=width,
                                 device=device,anchor_c2w=_geo_anchor_c2w),
                            dict(latents_mean=latents_mean,latents_std=latents_std,
                                 latent_window_size=latent_window_size,device=device,generator=generator,
                                 sigma_min=_vsnoise_sigma_min,sigma_max=_vsnoise_sigma_max,
                                 visibility_aware_noise=_vsnoise_enabled,sigma_invisible=_vsnoise_sigma_invisible))
                    else:
                ...
                if _use_raw_sink_frames:
                    sink_latents = _latents_prefix_clean
                    if is_first_section:
            ...
            if _live_geometry is not None:_live_geometry.start_dit()
            _t_diff0 = _ptime.time()
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                if is_enable_stage2:
                    ...
                    _cam_Ks_chunk = None
                    if cam_c2ws_dev is not None:
                        _s_pix = geo_ref_pose_offset_pix + _geo_pose_k * latent_window_size * self.vae_scale_factor_temporal
                ...
                if _live_geometry is not None:_live_geometry.end_dit()
                ...
                if use_geometric_state and geo_state is not None:
                    ...
                    _geo_boundary = self._geo_quantize_8bit(_geo_chunk_decoded[:, :, -1])
                    geo_state["prev_chunk_last_frame"] = _geo_boundary


                    _geo_num_pix = int(_geo_chunk_decoded.shape[2])
                    _geo_pix_stride_add = latent_window_size * self.vae_scale_factor_temporal
                    _geo_chunk_pixel_start = geo_ref_pose_offset_pix + _geo_pose_k * _geo_pix_stride_add
                    _geo_add_n = int(getattr(self, "_geo_chunk_add_n", 1) or 1)
                    if _geo_add_n == 1:
                        _geo_frame_offsets = (_geo_num_pix - 1,)
                    elif _geo_add_n == 3:
                        _geo_frame_offsets = (0, _geo_num_pix // 2, _geo_num_pix - 1)
                    else:
                    ...
                    if (not is_event_k) and geo_state.get("da3_bank") is not None and geo_c2ws_dev is not None:
                        _ingest_started = _ptime.perf_counter()
                        if _live_geometry is not None:
                            _live_geometry.ingest(_geo_chunk_decoded[0],int(_geo_chunk_pixel_start),geo_c2ws_dev)
                        else:
                            self._geo_da3_ingest(geo_state, _geo_chunk_decoded[0],
                                                 int(_geo_chunk_pixel_start), geo_c2ws_dev, device)
                            if live_control is not None:
                                live_control.ingest_seconds += _ptime.perf_counter() - _ingest_started
            ...
            if live_control is not None:
                live_control.chunk_done(warp_seconds=_t_warp_s, diffusion_seconds=_t_diff_s)


                history_latents = history_latents[:, :, -max(sum(history_sizes), latent_window_size):].clone()
...
def _oracle_gt_fetch(cache, pix_a, pix_b, height, width, ref_video=None):
    ...
    try:
        ...
        import numpy as _np
        fr = torch.from_numpy(_np.asarray(vr.get_batch(idxs))).float()
        v = ((fr / 127.5) - 1.0).permute(0, 3, 1, 2)
        _, _, h, w = v.shape
